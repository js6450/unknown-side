			...
			for (auto& skeleton : bodySkeletons)
			{
				...
				if (recordData == true) {
					for (int i = 0; i < K4ABT_JOINT_COUNT; ++i)
					{
						...
						if (i == 27) {
							ofDrawBitmapStringHighlight(ofToString(bodyIndex), joint.position.xyz.x, joint.position.xyz.y + 15);
						}

						bodyFile << joint.position.xyz.x;
						bodyFile << ",";
						bodyFile << joint.position.xyz.y;
						bodyFile << ",";
						bodyFile << joint.position.xyz.z;
						bodyFile << ";";
					}
					bodyFile << "\n";
				}
...
void ofApp::keyPressed(int key) {
	...
	cout << "recording status ";
	cout << recordData << endl;

	if (recordData == true) {
		bodyFile.open(ofGetTimestampString() + ".txt", ofFile::WriteOnly);
	}
}
